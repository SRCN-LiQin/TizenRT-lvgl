...
{
    lv_tabview_set_tab_act(tv, id, false);
}
...
void lv_test_theme_1(lv_theme_t * th)
{
    ...
    create_tab2(tab2);
    create_tab3(tab3);
	//lv_tabview_set_tab_act(tv, 2, false);
}
...
static void create_tab1(lv_obj_t * parent)
{
    ...
    h_style.body.padding.inner = 4;//LV_DPI / 4;
    h_style.body.padding.hor = 4;//LV_DPI / 4;
    h_style.body.padding.ver = 4;//LV_DPI / 6;

    lv_obj_t * h = lv_cont_create(parent, NULL); //w 320, height 240-75
    lv_obj_set_style(h, &h_style);
    lv_obj_set_click(h, false);
    lv_cont_set_fit(h, false, false);
    //lv_cont_set_layout(h, LV_LAYOUT_PRETTY);
    lv_obj_set_pos(h, 0,0);
	lv_obj_set_size(h, 150,180);
	...
	lv_obj_set_size(label, 60,20);
	lv_obj_set_pos(label, 0,115);

    label = lv_label_create(h, NULL);
    lv_label_set_text(label, "Hint");
    lv_obj_set_style(label, th->label.hint);
	lv_obj_set_size(label, 60,20);
	lv_obj_set_pos(label, 0,140);
#endif
	//Button Matrix
    static const char * btnm_str[] = {"1", "2", "3", SYMBOL_OK, SYMBOL_CLOSE,""}; //last empty str is mandatory
    lv_obj_t * btnm = lv_btnm_create(h, NULL);
    lv_obj_set_size(btnm, LV_HOR_RES / 4, 2 * LV_DPI / 4);
    lv_btnm_set_map(btnm, btnm_str);
    lv_btnm_set_toggle(btnm, true, 3);
	lv_obj_set_pos(btnm, 5,110);

printf("1st content x %d, y %d, size %d, %d\n",lv_obj_get_x(h), lv_obj_get_y(h),lv_obj_get_width(h),lv_obj_get_height(h));

    h = lv_cont_create(parent, h);
    lv_obj_set_pos(h, 120, 0);
	lv_obj_set_size(h, 200,180);
    ...
    lv_obj_set_size(sw_h, LV_HOR_RES / 4, 50);
    //lv_cont_set_layout(sw_h, LV_LAYOUT_PRETTY);

    lv_obj_t * sw = lv_sw_create(sw_h, NULL);
	lv_obj_set_size(sw, 35, 15);
	lv_obj_set_pos(sw, 5, 5);

    sw = lv_sw_create(sw_h, sw);
    lv_obj_set_pos(sw, 5, 30);
    lv_sw_on(sw);

#if 0  // progress bar
    lv_obj_t * bar = lv_bar_create(h, NULL);
    lv_bar_set_value(bar, 70);
    lv_obj_set_size(bar, 70, 15);
	lv_obj_set_pos(bar, 5, 55);
#endif

    lv_obj_t * slider = lv_slider_create(h, NULL);
    lv_bar_set_value(slider, 60);
    lv_obj_set_size(slider, 60, 30);
	lv_obj_set_pos(slider, 5, 70);
	...
	lv_obj_set_size(ta, 120, 50);
	lv_obj_set_pos(ta, 5, 125);
	...
	lv_obj_set_size(ddlist, 60, 120);
	lv_obj_set_pos(ddlist, 70, 0);

printf("2nd content x %d, y %d, size %d, %d\n",lv_obj_get_x(h), lv_obj_get_y(h),lv_obj_get_width(h),lv_obj_get_height(h));
return;
...
printf("3rd content x %d, y %d, size %d, %d\n",lv_obj_get_x(h), lv_obj_get_y(h),lv_obj_get_width(h),lv_obj_get_height(h));
#endif
...
static lv_res_t keyboard_open_close(lv_obj_t * text_area)
{
    ...
    if(kb) {
        ...
        lv_obj_set_size(kb, w - 20, LV_VER_RES / 3);
        lv_obj_align(kb, ta, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 5/*LV_DPI*/);
        lv_kb_set_ta(kb, ta);
...
static void create_tab2(lv_obj_t * parent)
{
    ...
    lv_obj_set_pos(chart, LV_DPI - 20, 0);
    ...
    lv_obj_align(gauge, chart, LV_ALIGN_OUT_RIGHT_TOP, 5, 0);
    ...
    lv_obj_set_size(ta, w / 4, LV_VER_RES / 4);
    lv_obj_align(ta, NULL, LV_ALIGN_IN_TOP_LEFT, 0, 40);
...
static void create_tab3(lv_obj_t * parent)
{
    ...
    lv_obj_set_size(cal, 3 * LV_DPI, 185);
    //lv_obj_align(cal, NULL, LV_ALIGN_OUT_BOTTOM_MID, 0, -10);
    lv_obj_set_pos(cal, 0, -5);
