...
lv_fs_res_t lv_ufs_open(void *file_p, const char *fn, lv_fs_mode_t mode)
{
    ...
    if (ent == NULL) {
        if ((mode & LV_FS_MODE_WR) !=
            ...
            if (ent == NULL)
                return LV_FS_RES_FULL; /*No space for the new file*/
        } else {
 ...
 *         any error from lv__fs_res_t enum
 */
lv_fs_res_t lv_ufs_create_const(const char *fn, const void *const_p,
                                uint32_t len)
{
    lv_ufs_file_t file;
    lv_fs_res_t   res;
 ...
 *         any error from lv__fs_res_t enum
 */
lv_fs_res_t lv_ufs_write(void *file_p, const void *buf, uint32_t btw,
                         uint32_t *bw)
{
    lv_ufs_file_t *fp = file_p; /*Convert type*/
    *bw               = 0;
    ...
    if (new_size > ent->size) {
        ...
        if (new_data == NULL)
            return LV_FS_RES_FULL; /*Cannot allocate the new memory*/
...
lv_fs_res_t lv_ufs_seek(void *file_p, uint32_t pos)
{
    ...
    if (pos < ent->size) {
        ...
        if (new_data == NULL)
            return LV_FS_RES_FULL; /*Out of memory*/
